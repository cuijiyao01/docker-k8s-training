...
You can take any String as a master password, but if you want a random string you could do e.g. `openssl rand -base64 15 | base64` which will already give you a random already encoded password. (The first `-base64` option is used to only have alphanumerics in the password). Save the file `ads-db-secret.yaml`.
...
- After successful creation of the **Statefulset** check, wether the **Pod** `ads-db-pod-stateful-0` got created properly  via `kubectl get pod ads-db-pod-0 -o yaml` or in more detail via `kubectl describe pod ads-db-pod-stateful-0` . Also check wether the Database is ready to be connected via `kubectl logs ads-db-stateful-0`. There should be the line: `LOG:  database system is ready to accept connections` in the logs. 
